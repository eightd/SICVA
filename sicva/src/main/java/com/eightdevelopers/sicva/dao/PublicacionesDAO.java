...
public class PublicacionesDAO {
		...
		try {
			try {
				...
				if (num == 1) {
					...
					return "Error al guardar, publicaciones existente";
	...
	public List<PublicacionesDTO> listarasistenciacursos() {
		...
		try {
			...
			conexionBD.abrir();
			Connection connection = conexionBD.getConexion();
			PreparedStatement ps = connection.prepareStatement("SELECT * FROM publicaciones");
	...
	public String ActualizarLicenciatura(PublicacionesDTO asistenciacursosDTO) {
		try {
			try {
				ConexionBD conexionBD = new ConexionBD();
				conexionBD.abrir();
