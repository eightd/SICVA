	...
	public List<EscolaridadDTO> ListarEscolaridades() {
		...
		try {
			...
			while (rs.next()) {
				...
				escolaridadesDTO.setFechamodif(rs.getString("fecha_modificacion"));
	...
	public String ActualizarEvidencia(EscolaridadDTO escolaridadDTO, int idUsuMod) {
		try {
			try {
				...
				callableStatement.setBinaryStream(5, escolaridadDTO.getEvidencia().getInputstream());
				callableStatement.execute();
				Integer num = callableStatement.getInt(1);
				connection.close();
				if (num == 1) {
	...
	public String ActualizarEscolaridad(EscolaridadDTO escolaridadDTO, int idUsuMod) {
		try {
			try {
				...
				callableStatement.setInt(10, escolaridadDTO.getIdlic());
				callableStatement.setInt(11, escolaridadDTO.getIdusuario());
